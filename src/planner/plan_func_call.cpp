...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower" || func_name == "upper") {
    if (args.size() != 1) {
      throw Exception("Invalid number of arguments for function call " + func_name);
    }
    StringExpressionType v = StringExpressionType::Lower;
    if (func_name == "upper") {
      v = StringExpressionType::Upper;
    }
    return std::make_shared<StringExpression>(args[0], v);
  }
