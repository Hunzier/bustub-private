  ...
  */
  ...
  for (auto request : lock_request_queue->request_queue_) {  // NOLINT
    if (request->txn_id_ == txn->GetTransactionId()) {
      if (request->lock_mode_ == lock_mode) {  // 如果现在已经是了的
        lock_request_queue->latch_.unlock();
        return true;
      }

      // 判断当前资源上是否有另一个事务正在尝试升级
      // 为了降低实现的复杂度，增加一条升级队列可以实现同步的升级
      ...
      // 找到第一个未被授予的锁请求的位置，即找到锁请求队列中第一个未获得锁的锁请求。
      ...
      // 将新的升级锁请求 upgrade_lock_request
      // 插入到锁请求队列中，位置为之前找到的第一个未获得锁的锁请求位置之前。这样可以保证升级锁请求优先于其他等待锁的请求。
      lock_request_queue->request_queue_.insert(lr_iter, upgrade_lock_request);
      // 将资源的 upgrading_ 字段设置为当前事务的ID，表示当前资源正在进行升级操作。
      lock_request_queue->upgrading_ = txn->GetTransactionId();

      std::unique_lock<std::mutex> lock(lock_request_queue->latch_, std::adopt_lock);
      // GrantLock检查当前请求的锁是否可以被授予，同时处理已经被授予的锁请求，以确保锁请求的合理性和一致性
